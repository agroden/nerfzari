...
from GameEngine import GameType
import GameEngine
...
class TestGameSetup(unittest.TestCase):
	...
	def test_game_types(self):
		...
		nerf_assassin = GameEngine.new_game(GameType.NERF_ASSASSIN, "Nerf Assasin Test Game 0" , datetime.now())
		self.assertIsNotNone(nerf_assassin)
		self.assertIsInstance(nerf_assassin, NerfAssassin)

		#team_assassin = GameEngine.new_game(GameType.TEAM_ASSASSIN, "Team Assassin Test Game 0" , datetime.now())
		#self.assertIsNotNone(team_assassin)
		#self.assertIsInstance(team_assassin, TeamAssassin)

		#nerf_zombie = GameEngine.new_game(GameType.NERF_ZOMBIE, "Nerf Zombie Test Game 0" , datetime.now())
